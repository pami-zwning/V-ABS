...
def parse_json_from_response(response: str):
    ...
    try:
        return {'status': True, 'content': json.loads(content)}
    except:
        pass
    ...
    # If no code block found, try to find curly braces
    if not json_str:
        start = content.find("{")
        end = content.rfind("}")
    ...
    if not json_str:
        return {'status': False, 'message': 'No JSON found'}

    try:
        # Try to fix common single-quote errors
        ...
        try:
            ...
            return {'status': False, 'message': f"JSON Decode Error: {e}"}
