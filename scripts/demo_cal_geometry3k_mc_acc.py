...
def process_one_task(task_output_dir):
    try:
        ...
        ex = load_json(ex_json_path)

        ex_label = float(ex['choices'][['A', 'B', 'C', 'D'].index(ex['answer'])])

        choices = [float(choice) for choice in ex['choices']]
        ...
        # if abs(output_answer - ex_label) < 1e-2 * ex_label:
        #     return 1 
        # else:
        #     return 0
        min_error = min([abs(output_answer - choice) for choice in choices])
        gt_error = abs(output_answer - ex_label)
        if min_error < gt_error + 1e-9:
            return 1
        else:
            return 0
